...
- **转换执行阶段**用到 GPU（Marker + surya-ocr，torch cu128），你的显卡是 RTX 5060 / 8GB 显存（**[订正 2026-07-01]** 家用机实测为 **RTX 4060 / 8GB**；此处 5060 系公司机 session 记录，两机显存同为 8GB，结论不受影响）。3 路并行跑 GPU 推理大概率会挤爆显存——具体并发数以冒烟测试实测的显存占用为准，默认先按 1 路跑，观察显存余量后再决定是否提到 2 路。不建议在没实测前就按"3 批并行"跑转换阶段。
