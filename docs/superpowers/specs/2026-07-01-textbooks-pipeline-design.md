...
scripts/pipelines/textbooks/          # 新建,与 patents/general 平级、零耦合
  convert.py        # 单文档: 分诊 → PDF→PNG → predict → 自建重组 → Typora md
  triage.py         # 输入分诊(文本层可信度判 A/B/C,见 §5.1)
  preprocess.py     # PDF→PNG 预处理(见 §5.2)
  reconstruct.py    # 从 parsing_res_list JSON 自建重组(见 §5.4)
  selfcheck.py      # Tier0 确定性自检(见 §8)
  debug_view.py     # 人工复核 HTML(见 §8.1,移植改造自 patents 版)
  batch.py          # 批量 + 分块 + 断点续跑(见 §7)
  README.md
...
### 5.1 输入分诊（triage，`triage.py`）

教科书输入混杂，**非全是纯扫描**。前置分诊按"文本层可信度"三分，首版只处理需 OCR 的两类：

- **A. 无文本层（纯扫描）** → 走 §5.2 起的 OCR 主路
- **C. 低质文本层（坏 OCR 层，文本本身有错）** → 弃文本层，同样走 OCR 主路（force OCR，从图重识别）
- **B. 优质文本层（born-digital，常几兆小文件）** → **首版不转，登记到 `_deferred_born_digital/`**，留待后续（见 §9）

两道闸（确定性，无 ML）：

1. **覆盖度闸（判 A vs 有层）**：采样页 `get_text` 平均字符数，近 0 → A 纯扫描。沿用
   [批量转换方案](2026-07-01-literature-batch-conversion-design.md) §3 已有的文本层探测。
2. **质量闸（判 B vs C）**：有文本层时算"坏度信号"——U+FFFD(�)/私用区/控制字符比例、字体 ToUnicode/CID
   缺失比例、非词率（借 patents `wordfix` 的 82k 英文词表 + 中文词典）、坏字形模式（借 patents
   [garblecheck.py](../../../scripts/pipelines/patents/garblecheck.py) 的 CaSOS/ringS 类 OCR 误读检测）。
   综合超阈值 → C（弃层重 OCR）；否则 → B（优质，登记不转）。**阈值用样本实测标定**，实现阶段做。

分诊结果写入 manifest（见 §7 断点续跑），每份文档标 A/B/C 与判据得分，可审计、可人工复核纠偏。

### 5.2 PDF → PNG 预处理（实证必需）
...
- **B 路：优质文本层 born-digital 教科书的文本层直取**（pymupdf4llm 或路由 general）：首版只分诊登记不转，此路径留待后续。注意 pymupdf4llm 对公式/复杂版面较弱，若 born-digital 公式是图片可能漏。
