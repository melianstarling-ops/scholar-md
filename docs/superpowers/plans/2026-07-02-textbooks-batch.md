...
### Task 1: `convert_pdf()` 加 `write_selfcheck` 参数 + CLI `--no-selfcheck-json`
...
**Interfaces:**
- Consumes: 无新依赖，仅在既有 `convert.py` 内加参数。
- Produces: `convert_pdf(pdf_path, out_dir=None, dpi=cp.DEFAULT_DPI, write_selfcheck=True) -> dict`（新增
  `write_selfcheck` 关键字参数，行为不变时其余签名/返回值不变）；副作用：`write_selfcheck=True`（默认）时在
  `doc_out/<stem>_selfcheck.json` 写入与返回值里 `selfcheck` 字段相同的 JSON。CLI 新增 `--no-selfcheck-json`。
  Task 2（watchdog.py）与 Task 5（batch.py）依赖这个 CLI flag 名字原样透传。

- [ ] **Step 1: 写失败测试——`write_selfcheck=True`（默认）落盘且内容与返回值一致**
...
Run: `.venv-textbooks/Scripts/python -m pytest scripts/pipelines/textbooks/tests/test_convert.py -k selfcheck_json -v --basetemp=./.pytest_tmp`
Expected: 3 个新测试 FAIL（`write_selfcheck` 未知关键字参数 / 文件不存在断言失败 / `--no-selfcheck-json` 未知 CLI 参数）。
...
把 `main()`（原第 122-137 行）里的 argparse 部分：
...
Run: `.venv-textbooks/Scripts/python -m pytest scripts/pipelines/textbooks/tests/test_convert.py -v --basetemp=./.pytest_tmp`
...
git commit -m "feat(textbooks): convert_pdf 加 write_selfcheck 参数,落盘 <stem>_selfcheck.json"
```

---

### Task 2: `watchdog.py` 透传 `--no-selfcheck-json`
...
**Interfaces:**
- Consumes: Task 1 的 `convert.py --no-selfcheck-json` CLI flag（原样字符串透传，不解析其语义）。
- Produces: `watchdog.py` CLI 新增 `--no-selfcheck-json`（`store_true`），指定时追加到传给
  `run_until_done` 的 argv 末尾。`run_until_done()` 函数签名不变。
...
Run: `.venv-textbooks/Scripts/python -m pytest scripts/pipelines/textbooks/tests/test_watchdog.py -k no_selfcheck_json -v --basetemp=./.pytest_tmp`
Expected: FAIL（`--no-selfcheck-json` 未知 CLI 参数）。
...
Run: `.venv-textbooks/Scripts/python -m pytest scripts/pipelines/textbooks/tests/test_watchdog.py -v --basetemp=./.pytest_tmp`
Expected: 全部 PASS。
...
git commit -m "feat(textbooks): watchdog.py 透传 --no-selfcheck-json 给 convert.py 子进程"
```

---

### Task 3: `batch.py` 骨架 + `discover()`（自适应展开 + 跨目录同名硬失败）
...
### Task 4: `_already_done()`（`--resume` 判断：指纹/DPI 校验 + 毒页感知）
...
- Produces: `_already_done(out_root: Path, pdf_path: Path, dpi: int) -> bool`。Task 5 的 `run()` 用它做
  `--resume` 跳过判断。
...
**Interfaces:**
- Consumes: Task 3 的 `discover()`、Task 4 的 `_already_done()`、`watchdog.run_until_done(argv, max_restarts, runner=None) -> int`（已存在，`runner` 参数已支持注入）、`checkpoint.load_manifest`。
...
把 `scripts/pipelines/textbooks/batch.py` 顶部导入区（Task 3/4 累积后的状态）：
...
- §2 进程隔离模型 → Task 5 `run()` 用 `run_until_done(..., runner=runner)`。✓
- §3 目录布局(不做 --flat) → 全程 `out_root / pdf.stem`，未新增平铺分支。✓
- §4 selfcheck.json 落盘 → Task 1(`convert_pdf`)/Task 2(`watchdog.py` 透传)。✓
- §5 CLI 全部 flag(`--src/--out/--dpi/--resume/--limit/--max-restarts/--no-selfcheck-json/--list`) → Task 5 `main()`。✓
- §5 跨目录同名 stem 硬失败 → Task 3 `discover()` + Task 5 `main()` 捕获 `ValueError`。✓
- §6 `--resume` 三条判断(B 不短路/指纹校验/毒页豁免) → Task 4 `_already_done()`；B 不短路体现在 `run()`
  只对 A/C 走 `_already_done`，从不检查 `_deferred_born_digital` 标记做跳过。✓
- §6 `--limit` 先于 `--resume` → Task 5 `run()` 先 `pdfs[:limit]` 再进循环判断跳过。✓
- §7 汇总报告 + rc 语义(仅 GIVEUP→1) → Task 5 `run()` 尾部统计 + 返回值。✓
- §8 测试策略(注入假 runner，全程无真子进程/GPU) → Task 5 测试全部用 `fake_runner`。✓
- §9 范围外(`--flat`/`--review`/`--ocr`/就地默认) → 计划全程未引入。✓

**Placeholder 扫描**：全部步骤含完整代码块，无 TBD/"适当处理"/"参考 Task N 类似实现"。

**类型一致性**：`_already_done(out_root: Path, pdf_path: Path, dpi: int) -> bool` 在 Task 4 定义、Task 5
`run()` 内以 `_already_done(out_root, pdf, dpi)` 原样调用，参数顺序/类型一致。`run()` 返回
`tuple[int, list[dict]]`，Task 5 测试解包 `rc, results = bp.run(...)` 与 `main()` 内 `rc, _ = run(...)` 一致。
`discover()` 返回 `list[Path]`，Task 4/5 测试与实现均以 `Path` 对象操作（`.stem`），无 str/Path 混用。
