...
| `debug_view.py` | 可视化调试：左=页面图+判定叠加层（行号/页眉/保留词/段落/crosscheck 告警，逐层显隐+缩放），右=该页重排中间产物；双向联动；标记模式（四类语义 + 文字 note）；多轮调和（自动嵌入归档键集，已处理标记 ✓ + 一键清除；导出按钮带未导出角标）；暗色默认可切换。导出两模式：`--serve` 直写工作区（推荐）/ 静态文件 + `--collect` 归位 |
| `check_annotations.py` | 标记回归断言：把已修复的标记当永久回归用例——重跑引擎现算，wrong_del 断言词回 kept 且在 md、missed_del 断言词进 removed；conv_err/区域标记列人工复核。改判定逻辑后必跑（与 Tier0/crosscheck 并列） |
...
# 可视化调试视图。两种模式:
#  ① 服务模式(推荐): 导出标记直接回写 03_Output/<专利>/,无弹窗/无 Downloads/无 --collect。
#     运行后在 VS Code Simple Browser 或浏览器打开打印的 http://127.0.0.1:8077/
.venv\Scripts\python.exe scripts\pipelines\patents\debug_view.py --serve
#  ② 静态模式: 生成 <stem>_debug.html(脚本同目录),双击/预览打开;导出落 Downloads,再归位:
.venv\Scripts\python.exe scripts\pipelines\patents\debug_view.py
.venv\Scripts\python.exe scripts\pipelines\patents\debug_view.py --collect   # 下载目录标记归位
# 先跑 crosscheck 可叠加红色告警层;标记导出 <stem>_annotations.json 交 agent 修引擎

# 标记回归断言(改引擎判定逻辑后必跑)
.venv\Scripts\python.exe scripts\pipelines\patents\check_annotations.py
...
# 外部工作区调用（产物落外部目录，本仓零污染）
# --src              : 外部 PDF 源目录（或设环境变量 SCHOLARMD_PATENTS_SRC）
# --out              : 外部产物根目录
# --flat             : 平铺模式 — md 和 artifacts/ 直接落 --out 根，不建每篇子目录
# --no-selfcheck-json: 不写 _selfcheck.json（控制台 OK/SUSPECT 摘要仍输出）
# 脚本目录自动入 sys.path，无需改 PYTHONPATH
.venv\Scripts\python.exe scripts\pipelines\patents\batch_patents.py `
    --src D:\MyPatents\pdfs `
    --out D:\MyPatents\output `
    --flat --no-selfcheck-json
# 产物结构（--flat --no-selfcheck-json）:
#   output/Patent_A.md
#   output/Patent_A_artifacts/fig_p001.png ...
#   output/Patent_B.md ...
# 含 OCR 前置 + 增量 resume：
.venv\Scripts\python.exe scripts\pipelines\patents\batch_patents.py `
    --src D:\MyPatents\pdfs `
    --out D:\MyPatents\output `
    --flat --no-selfcheck-json --ocr --resume
```
