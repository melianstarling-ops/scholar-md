...
| `debug_view.py` | 可视化调试（自包含单 HTML，默认输出到本目录、已 gitignore）：左=页面图+判定叠加层（行号/页眉/保留词/段落/crosscheck 告警，逐层显隐+缩放），右=该页重排中间产物；双向联动；标记模式（四类语义 + 文字 note → 导出 `*_annotations.json`，`--collect` 归位）；多轮调和（自动嵌入归档键集，已处理标记 ✓ 显示 + 一键清除；导出按钮带未导出角标）；暗色默认可切换 |
