...
2) 结构 lint：残留行号、页眉泄漏、claims 编号连续性、FIG 引用可解析。

产出报告 dict；missing 比例超阈值或有结构问题即标记为需人工/AI 复核。
"""
...
def lint(output_text: str, claims_md: str, fig_labels_all: list[str], profile: LayoutProfile) -> list[str]:
    ...
    if unresolved and have:
        issues.append(f"正文引用但未找到对应附图: FIG {sorted(unresolved)[:8]}")
...
def run(
    ...
    issues = lint(lint_text if lint_text is not None else final_markdown, claims_md, fig_labels_all, profile)
