...
def _page_chars(path) -> tuple[int, int, int, bool, str, tuple[str, ...]]:
    ...
    has_visual = any(isinstance(block, dict) and block.get("block_label") in _VISUAL_LABELS
                     for block in blocks)
    return page_number(path), len(text.strip()), len(ordered), has_visual, text, contents
...
def detect_novel_signals(context: DetectorContext) -> list[Finding]:
    ...
    positive = [chars for _, chars, _, _, _, _ in stats if chars > 0]
    if len(positive) < 3:
        return []
    median = statistics.median(positive)
    if median < 100:
        return []
    by_page = {page: chars for page, chars, _, _, _, _ in stats}
    findings: list[Finding] = []
    for page, chars, blocks, has_visual, text, contents in stats:
        previous = by_page.get(page - 1, 0)
        following = by_page.get(page + 1, 0)
        if (blocks and not has_visual and chars < max(20, median * 0.15)
