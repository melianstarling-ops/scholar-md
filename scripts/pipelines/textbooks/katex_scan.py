...
def _work_pages_scan_md(layout: DocLayout, corrections_dir: str | None = None) -> str:
    ...
    corrections = load_corrections(corrections_dir or layout.doc_work_dir)
    pages: list[str] = []
    for page in range(1, total + 1):
        blocks = cp.load_page_blocks(layout.work_dir, page)
        if corrections:
            blocks = apply_corrections(blocks, page, corrections)
...
def scan_katex_work_pages(layout: DocLayout, out_path: str, node_bin: str | None = None,
                          timeout: int = 120,
                          corrections_dir: str | None = None) -> dict | None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            f.write(_work_pages_scan_md(layout, corrections_dir=corrections_dir))
...
def main() -> None:
    ...
    ap.add_argument("--corrections-dir", default=None,
                    help="修正层所在文档目录(默认沿用 work-dir 下的文档目录)")
    args = ap.parse_args()
    if args.work_dir and args.deliverables_root and args.stem:
        ...
        result = scan_katex_work_pages(
            layout, args.out, corrections_dir=args.corrections_dir)
    elif args.md:
