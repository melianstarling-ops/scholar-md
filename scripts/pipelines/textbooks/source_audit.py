...
class AuditThresholds:
    ...
    # 定位样本上限(计划 §6.4:missing_samples/added_samples "最多保存少量
    # 定位样本,不把全文重复写入 JSON")。带默认值,追加在末尾——不破坏既有
    # 按位置参数构造 AuditThresholds 的调用点。
    maximum_samples_per_block: int = 3
...
def _diff_run_samples(
    source_tokens: list[str], ocr_tokens: list[str], max_samples: int
) -> tuple[list[str], list[str]]:
    """逐 token diff(计划 §6.4),取连续缺失/新增 run 作定位样本。

    用 difflib.SequenceMatcher 对齐 source_tokens/ocr_tokens:delete/replace
    段的源侧 token 是"源有而 OCR 无"(missing),insert/replace 段的 OCR 侧
    token 是"OCR 有而源无"(added)。每条样本截断到 _SAMPLE_MAX_CHARS 字符,
    各自最多保留 max_samples 条——绝不把整页/整块原文塞进去。
    """
    matcher = difflib.SequenceMatcher(a=source_tokens, b=ocr_tokens, autojunk=False)
    missing: list[str] = []
    added: list[str] = []
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag in ("delete", "replace") and i2 > i1:
            missing.append(" ".join(source_tokens[i1:i2])[:_SAMPLE_MAX_CHARS])
        if tag in ("insert", "replace") and j2 > j1:
            added.append(" ".join(ocr_tokens[j1:j2])[:_SAMPLE_MAX_CHARS])
    return missing[:max_samples], added[:max_samples]
...
def audit_prose(
    ...
    (按 block_id;触发 missing_prose/prose_mismatch 的块另带
    missing_samples,触发 ocr_addition 的块另带 added_samples——各自最多
    thresholds.maximum_samples_per_block 条、每条截断到 ~80 字符的定位样本,
    绝不把整块/整页原文写进去;无对应 issue 的块不产该字段)。
    """
    ...
    for i, block in enumerate(blocks):
        ...
        # 见测试)。惰性求值:先算好,命中哪类 issue 再决定挂哪个键。
        missing_samples, added_samples = _diff_run_samples(
            source_tokens, ocr_tokens, thresholds.maximum_samples_per_block
        )
