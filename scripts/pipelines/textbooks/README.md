...
都接受 `--formula-repair {deterministic,agents,off}`,决定单本转换成功(md/
`_selfcheck.json` 已落盘)后自动往下接哪一段公式修复环:
...
- **`agents`** —— `deterministic` 全部 + 第 9 步公式 Agent 五道门(冻结模型链,
  外部 LLM 调用)。**红线**:编排层把调用 `run_agents` 的 `mode` 硬编码为
  `"propose"`,不依赖 orchestrator 内部熔断/闸门"恰好"把它降级——corrections
  只落 `status=pending`,**绝不自动 accept/应用**,md 一字不改。adapters 全不
  可用(未装 CLI/未登录)时优雅降级为 `deterministic` 行为(不调用任何 agent,
  只留明确记录),不崩。
...
一遍)。因此:batch 显式选 `--formula-repair {deterministic,agents}` 时才转发
给子进程,并自动让路(跳过)batch 自己对应的收尾步骤,不管 `--no-katex-scan`
传的是什么——两边加起来仍是"跑一遍",不双跑。

```bash
# 默认(deterministic):KaTeX 扫描/分桶/候选漏斗全自动,零网络零成本
$V -m scripts.pipelines.textbooks.convert --src book.pdf --out ./out

# agents:额外接公式 Agent 终检链,corrections 只落 pending,人工确认门不变
...
> 默认行为。经 `convert.py --formula-repair agents`(或 `watchdog.py`/
> `batch.py` 转发)自动接入时,编排层把调用模式硬编码为等价 `--propose`——
> corrections 只落 `pending`,不会自动应用/改 md,人工确认门(第 7 步)始终
> 是唯一能让修正进 md 的路径。要"全自动应用"必须像上面这样独立、显式地跑
> `formula_agents.cli`(不带 `--propose`/`--dry-run`)。
