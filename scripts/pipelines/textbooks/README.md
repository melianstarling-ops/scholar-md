...
- **`agents-apply`** —— `deterministic` 全部 + 第 9 步公式 Agent 五道门,以
  ...
  adapters 全不可用时同样优雅降级为 `deterministic` 行为,不崩。熔断/回滚一旦
  触发,该本书的 corrections 会悄悄退回 `status=pending`——`agents-apply` 的
  设计本就不留人工 accept 门,没人会去审——落盘的 `<stem>_formula_repair.json`
  与 `batch.py` 汇总行的 `fr=circuit_broken`/`fr=rolled_back` 分诊标记就是为
  这种情况准备的,批量跑完务必按标记回查。
